...
@Component({
  selector: 'app-docsfind',
  templateUrl: './docsfind.component.html',
  styleUrls: ['./docsfind.component.scss']
})
export class DocsfindComponent implements OnInit {
  ...
  findDoc(){
    ...
    let client:string =this.control.value?.nit;
    let document:string =this.controlDoc.value;

    this.label="Obteniendo detalles de factura";
    this.traza.Getfacab(client,document).subscribe({
      next:(res)=>{
        console.log(res.data);
        this.FacturaCabeza=res.data.fac;
      ...
      complete:()=>{
        this.label="Obteniendo imagenes de factura";
        this.cdr.detectChanges();
        this.imgservice.getImages(client,3,document).subscribe({
          next:(response)=>{
          ...
          complete:()=>{
            this.label="Obteniendo información de masivo";
            this.cdr.detectChanges();
            this.traza.GetMasivo(client,document).subscribe({
              next:(res)=>{
                if(res.data.ossm.length>0){
                  this.FacturaCabeza=res.data.ossm;
                  this.cdr.detectChanges();
                }
              },
              error:()=>{
              ...
              complete:()=>{
                this.label="Obteniendo pedidos de factura";
                this.cdr.detectChanges();
                this.traza.GetPedfac(client,document,Constant.AUTH.getAgency()?.vus_codage).subscribe({
                  next:(res)=>{
                    if(res.data.ped.length>0){
                      this.PedidosFac=res.data.ped;
                      this.cdr.detectChanges();
                    }
                  },
                  error:()=>{
                  ...
                  complete:()=>{
                    this.label="Obteniendo ordenes de despacho";
                    this.cdr.detectChanges();
                    this.traza.GetOd(client,document,Constant.AUTH.getAgency()?.vus_codage).subscribe({
                      next:(res)=>{
                        console.log(Constant.AUTH.getAgency()?.vus_codage);
                        if(res.data.ods.length>0){
                          this.OrdenesDes=res.data.ods;
                          this.cdr.detectChanges();
                        }
                      },
                      error:()=>{
                      ...
                      complete:()=>{
                        this.label="Obteniendo novedades";
                        this.cdr.detectChanges();
                        this.traza.GetNovedades(client,document,Constant.AUTH.getAgency()?.vus_codage).subscribe({
                          next:(res)=>{
                            console.log(Constant.AUTH.getAgency()?.vus_codage);
                            if(res.data.nov.length>0){
                              this.Novedades=res.data.nov;
                              this.NovedadesConf=res.data.conf;
                              this.cdr.detectChanges();
                            }
                          },
                          error:()=>{
                          ...
                          complete:()=>{
                            this.label="Obteniendo cumplidos";
                            this.cdr.detectChanges();
                            this.traza.GetCumplidos(client,document,Constant.AUTH.getAgency()?.vus_codage).subscribe({
                              next:(res)=>{
                                console.log(Constant.AUTH.getAgency()?.vus_codage);
                                if(res.data.cum.length>0){
                                  this.Cumplidos=res.data.cum;
                                  this.cdr.detectChanges();
                                }
                              },
                              error:()=>{
                              ...
                              complete:()=>{
                                ...
                                this.label="";
                                this.cdr.detectChanges();
                                this.toastr.success('Busqueda finalizada');
                                console.log(this.FacturaCabeza);
                              }
                            })
                          }
                        })
  ...
  onClear(){
    ...
    this.router.navigateByUrl('/', { skipLocationChange: true }).then(() => {
      this.router.navigate(['/trazability']);
  });
  }
}
